...
class Converter:
    ...
    def pyopenssl_cert_to_cryptography(self, pyopenssl_cert):
        ...
        if pyopenssl_cert.__class__ is crypto.X509:
            ...
            return openssl.x509._CertificateSigningRequest(openssl.backend, pyopenssl_cert._req)
        else:
            raise TypeError(f"Unknown input type: {pyopenssl_cert.__class__}")
    ...
    def pyopenssl_cert_to_json(self, pyopenssl_cert):
        ...
        valid_from = datetime.strptime(pyopenssl_cert.get_notBefore().decode("ascii"), "%Y%m%d%H%M%SZ")
        dt_format = "%Y-%m-%d"
        json_cert["valid_from"] = valid_from.strftime(dt_format)
        valid_till = datetime.strptime(pyopenssl_cert.get_notAfter().decode("ascii"), "%Y%m%d%H%M%SZ")
